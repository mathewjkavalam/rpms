...
Panel Members
"""
def isGuideIn(hr,day,guideslno,className = ["S8 CS B","S8 CSB"],projectperiodnamegiveninguidestt = ["PROJECT"]):
    ret = False
    if (hr > 1):
...
def isFreePeriod(hr,day,guideslno=1):
    around = twoCellsWithinPeriod(hr,day,guideslno)
    return ( guidestt.cell(column=classH[hr], row=guideday( workD[day],guideslno) ).value is  None ) and ( around.intersection(long3Periods) == set() )
...
for g in range(1,18+1):
    ...
    for d in [1,2,3,4,5]:
        for h in [1,2,3,4,5,6,7]:
            #print(h,d,g)
            if( isGuideIn(h, d, g,["S8 CS A","S8 CSA"]) ):
                ft.append( (h,d,'A') )

            elif( isGuideIn(h, d, g,["S8 CS B","S8 CSB"]) ):
                ft.append( (h,d,'B') )

            elif( isGuideIn(h, d, g,["S8 CS C","S8 CSC"]) ):
                ft.append( (h,d,'C') )
